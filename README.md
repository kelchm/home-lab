...
| Ingress | Traefik (Gateway API) — split `admin` / `services` gateways |
...
- **`identity`** — Kanidm as the OIDC provider / user directory, run by the
  kaniop operator.
- **`observability`** — a VictoriaMetrics + VictoriaLogs stack (and an
  OpenObserve / Grafana / kube-prometheus comparison), the subject of an
  ongoing [bake-off](docs/observability-bakeoff.md).
- **`ai`** — an MCP gateway (mcpjungle / metamcp) plus Playwright MCP servers.
