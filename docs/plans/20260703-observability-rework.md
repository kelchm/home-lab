...
**Status:** Active — 2026-09-06; convergence is complete. The VictoriaMetrics/VictoriaLogs survivor path is live and verified, and OpenObserve, Loki, and kube-prometheus-stack are removed, closing #470. This revision carries that deployed baseline into an estate-wide metrics, logs, flow, and security-observability design. Background: [observability bake-off](../observability-bakeoff.md).
...
- The survivor cutover is no longer proposed work: PRs #478, #480, and #481 completed the CRD, native scrape, rule, alert-delivery, and Grafana ownership transfers, and PR #482 recorded their successful live gates. PRs #526 and #528 then completed the soak and the KPS/Loki cleanup. The remaining Phase 1 work is the estate-label migration before external targets and independent dead-man coverage.
...
| External operational logs | PVE, Sparks, Synology | Native or explicitly installed syslog forwarding, or authenticated journald HTTP upload, to a restricted VictoriaLogs ingress | VictoriaLogs | Host, storage, authentication, and audit diagnosis |
...
8. Choose and fixture the external journal-forwarding mechanism for PVE 9 and confirm which syslog facility, if any, is already present on the Sparks. If PVE uses its journal-native option, pin it to `systemd-journal-upload` over authenticated HTTPS to a source-restricted `/insert/journald` route; provision its per-host credential, VMAuth mapping, VIP, and Cilium path explicitly rather than treating it as syslog.
...
#### Phase 1B — completed soak and rollback-stack cleanup

Completed by PRs #526 and #528 on 2026-09-06.

- **Soak.** Over the 24 hours before the fan-out was removed, both sinks delivered identically — 51,403 entries each in the final hour, with zero dropped entries on either endpoint. The 829,591 lifetime `ingester_error` drops on the Loki sink predate that window; they coincide with the default-deny rollout, during which Loki was unreachable and **VictoriaLogs took zero drops**. That is production evidence for what the planned "make Loki unreachable and measure whether the shared fan-out stalls VictoriaLogs" fault test was meant to establish, so the test was not re-staged before deleting the fan-out it would have exercised.
- **First commit (#526).** Removed Alloy's `loki.write` forward and `dependsOn: loki` together, retaining `dependsOn: victoria-logs-single`. `validate-alloy-log-paths.sh` previously asserted the dual-write fan-out as a convergence gate and now asserts the inverse: exactly one `loki.write` component, forwarding only to the VictoriaLogs receiver, with no reference to the Loki service. After reconciliation, every collector sent continuously with zero drops and zero retries, and the `loki.write.loki` component stopped reporting.
- **Second commit (#528).** Removed the Loki and kube-prometheus-stack manifests, the `prometheus.home.kelch.io` route and its OIDC redirect URI, the traefik-admin egress that backed it, the KPS scraper identity in three scrape-ingress policies, the `prometheus-community` HelmRepository, and both `dependsOn` entries. Added `Prometheus` and `Loki` to the surviving Grafana `deleteDatasources` list.
- **Coverage checked before deletion.** vmalert evaluated 286 alerting rules to KPS Prometheus's 65, with none unique to KPS. The VictoriaMetrics chart ships 43 dashboards to KPS's 17; every KPS title has a VM counterpart except "Grafana Overview", which renders empty because no target produces `grafana_build_info`.
- **Data-retention decision.** No rollback data is kept. Loki's 30 Gi claim is removed with its workload — its StatefulSet sets `whenDeleted: Delete` against a `Delete` reclaim policy. The KPS Prometheus 50 Gi, KPS Alertmanager 1 Gi, and orphaned OpenObserve 50 Gi claims are `Retain` and are deleted by hand once the uninstall is verified. VictoriaMetrics held metrics continuously across the cutover, so the KPS history is duplicative.
- **Accepted VictoriaLogs-outage behavior.** Alloy bounds VictoriaLogs retries at `max_backoff_retries=15` with no sender WAL, so an outage outlasting that budget drops entries. This is accepted rather than remediated: Loki was a parallel sink, never a backfill path, so its presence never mitigated this. The pinned `AlloyVictoriaLogsEntriesDropped` and `AlloyVictoriaLogsRetriesSustained` rules alert on the condition.

**Gate:** passed — one metrics path, one log path, and one notification authority remain, and no surviving resource depends on KPS or Loki.
...
1. Inventory every VictoriaMetrics and VictoriaLogs data-path reader and writer. The current clients are vmagent writing metrics and scraping the Alloy and VictoriaLogs `/metrics` endpoints that drive the live pipeline alerts, metric vmalert reading metrics and persisting state through its generated paths, Grafana reading VictoriaMetrics and VictoriaLogs, Alloy writing VictoriaLogs, authenticated human access through the existing OIDC routes, and explicit health probes. Future clients are the LogsQL evaluator reading VictoriaLogs and using VictoriaMetrics state paths, per-host PVE `systemd-journal-upload` writers if selected, an rsyslog/syslog-ng relay if selected, and the Phase 5 flow file-forwarding sidecar writing its isolated VictoriaLogs instance. The VictoriaMetrics operator reconciles Kubernetes resources through the Kubernetes API and is not assumed to be a backend data client without rendered or live evidence.
...
2. Forward selected PVE, Spark, and Synology logs using the fixture-tested mechanism chosen for each platform. PVE 9 requires either an explicit rsyslog installation to the syslog listener or `systemd-journal-upload` over authenticated HTTPS to its separately allowed `/insert/journald` route; Spark forwarding depends on the Phase 0 facility inventory; Synology uses its supported DSM log-forwarding facility.
...
1. Confirm the UDM Pro storage prerequisite and local Traffic Flows health, then capture and inventory a real IPFIX sample before finalizing the schema. Set the initial spool RPO to no more than five seconds of decoded records before persistence and no additional loss during rotation, with at least one hour of measured peak backlog plus 20% headroom across a restart; revise those numbers only through a recorded capacity decision before deployment.
2. Deploy the separate short-retention VictoriaLogs flow instance, GoFlow2, its dedicated authenticated file-forwarding sidecar, a size-bounded Longhorn RWO PVC spool with five-second-or-better flush behavior, retention and drain behavior, and a source-preserving VIP restricted to the UDM Pro. Use immutable size-bounded segments or a fixture-proven rename-and-reopen rotation; `copytruncate` is forbidden because copying an hour-scale active spool cannot preserve the five-second RPO. Keep operational container logs in the normal store and prove decoded payload does not leak there.
