...
- Alloy’s `/var/lib/alloy` hostPath currently persists file positions, not a durable VictoriaLogs outbound queue; no write-ahead log is enabled for the `loki.write` path. Define the accepted retry/drop RPO or enable and bound a supported WAL before Loki is removed, alert on the pinned Alloy version's verified retry and dropped-entry metrics, and test an outage longer than the configured retry window. The primary unmitigated risk is silent log loss, not automatic host-disk growth.
...
The current network policy intentionally blocks these cross-zone paths, so observability does not inherit reachability merely by deploying collectors. Before each platform is commissioned, update the firewall-policy matrix in the network-topology plan and the applied-rule record in `network/unifi/README.md` with narrowly ordered exceptions above the existing broad denies:

| Direction | Exact source | Exact destination | Purpose |
|---|---|---|---|
| Pull | `10.32.30.11-.13` | PVE `10.32.20.21-.23` | TCP 8006 for the PVE API and TCP 9100 for node-exporter |
| Pull | `10.32.30.11-.13` | Synology management `10.32.20.5` | UDP 161 for SNMPv3 |
| Pull | `10.32.30.11-.13` | Sparks `10.32.21.31-.32` | TCP 22 for the restricted SSH forwards plus only the separately declared inference metrics ports |
| Pull | `10.32.30.11-.13` | UDM Pro `10.32.1.1` | The verified UniFi controller API port for Unpoller |
...
These pulls deliberately grant the three Kubernetes node identities limited management-zone reach because pod egress is node-SNATed. UniFi cannot distinguish the approved collector pod from another pod after SNAT, so a Cilium egress policy must restrict each destination and port to the collector/vmagent service account before the matching UniFi allow is installed. Acceptance separately proves an unapproved pod is denied by Cilium and an unapproved VLAN is denied by UniFi.
...
- Run `prometheus-pve-exporter` centrally in Kubernetes. There is no PVE cluster VIP and round-robin DNS is intentionally forbidden, so query the `pve-cluster` class with cluster metrics enabled and node metrics disabled through all three API nodes. Retain the three raw target identities for source health, normalize duplicate cluster facts through `estate_*` recording rules for dashboards and alerts, and alert only when every cluster-class source is stale or failed. Use a separate `pve-node` class with cluster metrics disabled and node metrics enabled against each corresponding node.
...
Because `nv-monitor` intentionally omits TLS, bind its systemd listener explicitly to Spark loopback `127.0.0.1:9101`. A declared extra container in the vmagent pod maintains one SSH local forward to each Spark using a dedicated key and an account restricted to forwarding to that loopback endpoint. Both forward listeners bind only to the shared pod loopback namespace, so vmagent's bearer-authenticated HTTP request reaches the tunnel without crossing the cluster network in plaintext. The dedicated principal accepts SSH from the three node-SNAT addresses while the Sparks retain an additive host rule denying routed exporter access as defense in depth. Never use `flush ruleset` or otherwise replace Docker-managed nftables state; validate existing container networking after the additive rule. Test exporter bind address, `authorized_keys`/`sshd` forwarding restrictions, reconnect, host-key pinning, credential rotation, tunnel failure alerting, and five-second scrape behavior during a tunnel restart. If a reliable restricted SSH-forward account cannot be established, use a host TLS proxy and count it as an explicit exception to the one-binary posture rather than exposing a bearer token over HTTP.
...
- Use a separate small VictoriaLogs flow instance with 7–14-day retention so unknown flow volume cannot evict operational logs. GoFlow2 writes decoded JSON to a size-bounded shared spool rather than stdout, and a dedicated central sidecar forwards only that file to the flow instance. Specify rotation, retention, and rotation-aware tailing semantics; alert before the volume limit and prove rotation under sustained input without loss, duplication, or pod eviction. Preserve GoFlow2 and sidecar operational stdout/stderr in the 30-day store. Acceptance proves decoded payload never appears in CRI logs; add a narrowly container-scoped Alloy exclusion only as defense in depth if the selected image can emit payload to stdout.
...
Datasource convergence is completed baseline: retain `victoriametrics` as the default metrics UID and `alertmanager-vm` as the production Alertmanager UID. The explicitly named `prometheus-kps` datasource remains rollback-only until #470 cleanup; remove it through Grafana provisioning `prune`/`deleteDatasources` behavior rather than renaming or reusing its UID. Future dashboard work maps only to the production UIDs.
...
- Observe post-#483 Alloy targets for at least 24 continuous hours. Sample every node and include ordinary pods plus Talos mirror pods; require exact filename-to-label agreement, stable VictoriaLogs ingestion, zero unexpected send retries or drops, matching Loki/VictoriaLogs counts for named canary pods over bounded windows, and representative query correctness. Use only post-fix data for volume and stream-cardinality baselines.
...
1. Before enabling each listener or API/SNMP path, apply its deny-by-default host, UniFi, and Kubernetes network controls and prove the intended scraper succeeds while an unapproved VLAN and pod fail. Commission one platform at a time so no listener exists during an unprotected gap.
...
3. Install `node_exporter` on the three PVE hosts only after its source restrictions are active.
4. Install the single `nv-monitor` binary and systemd unit on both Sparks with the exporter bound to `127.0.0.1:9101`, add the pod-loopback SSH-tunnel extra container to vmagent with pinned host keys, add a dedicated forwarding-only monitoring account restricted to the three node-SNAT addresses and Spark loopback `:9101`, leave existing operator/automation SSH access unchanged, apply the additive routed-exporter denial before starting it, and enable the required inference `/metrics` endpoints.
5. Enable SNMPv3 on Synology with its source restriction already active and validate actual MIB coverage.
...
4. Deploy the selector-isolated LogsQL vmalert with VictoriaLogs datasource, VictoriaMetrics state storage, VMAlertmanager notifier, `evaluator=vmalert`, `signal=logs`, and a minimal first rule set.
...
**Gate:** source loss, parser failure, clock skew, and delivery failure are themselves observable; the deployed LogsQL evaluator uses its authenticated query and state paths; a synthetic LogsQL rule produces exactly one Pushover firing notification and one resolution through the non-null VMAlertmanager route; and no external source bypasses the authenticated internal read/write boundary.
...
2. Deploy the separate short-retention VictoriaLogs flow instance, GoFlow2, its dedicated file-forwarding sidecar, a size-bounded and rotation-tested shared spool, and a source-preserving VIP restricted to the UDM Pro. Keep operational container logs in the normal store and prove decoded payload does not leak there.
...
**Gate:** the flow pipeline has a volume budget, privacy decision, retention boundary, spool size limit, rotation and sustained-input proof, pod-eviction headroom, and tested loss and disk-growth alerts.
