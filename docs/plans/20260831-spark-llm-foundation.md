...
- Boot convergence made state-complete: after a reboot, `spark-baseline.service` takes the transaction lock, starts the default profile, resets the stable route to it, and converges the residency record, so neither a TP=2 route nor stale residency outlives the containers. TP=2 still never auto-starts.
