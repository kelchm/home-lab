...
Remote Admin is intentionally outside the older matrix. Its applied baseline is a dedicated zone with default blocks to Internal, VPN, Hotspot, DMZ, and itself; gateway and Internet access remain available. `Allow Main to Tailscale Routers` permits Main to reach only `10.32.19.101` and `.102`. `Allow Tailscale Routers to Routed LAN` permits those two sources to the six approved remote-access prefixes only: `10.32.1.0/24`, `10.32.10.0/24`, `10.32.20.0/24`, `10.32.30.0/24`, `10.32.130.0/24`, and `10.32.140.0/24`. Both policies have generated established/related return rules; all other Remote Admin → Internal initiation remains blocked.
