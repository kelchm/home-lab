...
- **Longhorn on NVMe**: dedicated user volume per node mounted at `/var/mnt/longhorn` (~890 GiB on the 1 TB SN770, xfs); 3-replica for critical PVCs (databases, stateful apps), 2-replica default. Replica engine ↔ replica engine traffic is intended to ride VLAN 25 (2.5GbE storage NIC) via Multus + macvlan; Longhorn's `storage-network` setting points at a NetworkAttachmentDefinition that attaches a secondary interface to instance-manager pods with IPs from the `.111-.119` Whereabouts pool (see [Storage VLAN specialization](#storage-vlan-specialization)). **Status**: Multus, Whereabouts, the NAD, and the macvlan binary installer are all deployed; the `storage-network` setting flip is the remaining cutover step, tracked in [`docs/runbooks/longhorn-storage-network-cutover.md`](runbooks/longhorn-storage-network-cutover.md). Until the cutover, replica traffic still rides VLAN 30 (1GbE pod network).
