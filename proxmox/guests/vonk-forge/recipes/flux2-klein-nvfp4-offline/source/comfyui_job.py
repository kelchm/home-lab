...
def server_command(temporary: Path) -> list[str]:
    """Generate exactly the CLI checked in the image before export."""
    ...
    return [
        ...
        "--database-url",
        "sqlite:///" + str(temporary / "comfyui.db"),
        "--user-directory",
        str(user_directory),
        "--temp-directory",
        str(temp_directory),
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="vonk-comfyui-") as temp_dir:
        ...
        command = server_command(temporary)
        environment = dict(os.environ)
        for name in ("HOME", "XDG_CACHE_HOME", "TRITON_CACHE_DIR", "TORCH_HOME", "TORCH_EXTENSIONS_DIR", "TORCHINDUCTOR_CACHE_DIR", "CUDA_CACHE_PATH", "HF_HOME", "PIP_CACHE_DIR", "CCACHE_DIR"):
