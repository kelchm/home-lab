...
Flatrate's dedicated gateway suppresses Hermes control chatter in Discord. Set `HERMES_GATEWAY_BUSY_ACK_ENABLED=false` in `/srv/hermes/profiles/flatrate/.env` so rapid follow-up messages are processed without an interrupt/redirect acknowledgment, and set `platforms.discord.gateway_restart_notification=false` in the live profile configuration so lifecycle operations do not post into the conversation. The similarly named top-level `discord.gateway_restart_notification` path is not consumed by the gateway and must not be used.
...
```sh
ssh kelchm@hermes.home.kelch.io 'set -e; test "$(grep -c "^MCPHUB_FLATRATE_DISCORD_TOKEN=" /srv/hermes/profiles/flatrate/.env)" -eq 1; test "$(grep -c "^HERMES_GATEWAY_BUSY_ACK_ENABLED=false$" /srv/hermes/profiles/flatrate/.env)" -eq 1; test "$(grep -c "^DISCORD_COMMAND_SYNC_POLICY=off$" /srv/hermes/profiles/flatrate/.env)" -eq 1; test "$(stat -c "%a" /srv/hermes/profiles/flatrate/.env)" = 600; ! grep -Eq "^(DISCORD_ALLOW_ALL_USERS|GATEWAY_ALLOW_ALL_USERS)=" /srv/hermes/profiles/flatrate/.env; test "$(sudo docker exec --user hermes hermes /opt/data/.local/bin/flatrate config get platforms.discord.gateway_restart_notification)" = false; test "$(sudo docker exec --user hermes hermes /opt/data/.local/bin/flatrate config get auxiliary.vision.provider)" = codex; test "$(sudo docker exec --user hermes hermes /opt/data/.local/bin/flatrate config get auxiliary.vision.model)" = gpt-5.6-luna; sudo docker exec --user hermes hermes /opt/data/.local/bin/flatrate auth status openai-codex | grep -q "logged in"; sudo docker exec --user hermes hermes /opt/data/.local/bin/flatrate mcp test mcphub_automotive_reference 2>&1 | grep -E "Connected|Tools discovered"'
```
