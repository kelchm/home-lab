...
```sh
ssh kelchm@hermes.home.kelch.io 'set -e; test "$(grep -c "^MCPHUB_FLATRATE_DISCORD_TOKEN=" /srv/hermes/profiles/flatrate/.env)" -eq 1; test "$(grep -c "^HERMES_GATEWAY_BUSY_ACK_ENABLED=false$" /srv/hermes/profiles/flatrate/.env)" -eq 1; test "$(stat -c "%a" /srv/hermes/profiles/flatrate/.env)" = 600; ! grep -Eq "^(DISCORD_ALLOW_ALL_USERS|GATEWAY_ALLOW_ALL_USERS)=" /srv/hermes/profiles/flatrate/.env; test "$(sudo docker exec --user hermes hermes /opt/data/.local/bin/flatrate config get platforms.discord.gateway_restart_notification)" = false; sudo docker exec --user hermes hermes /opt/data/.local/bin/flatrate mcp test mcphub_automotive_reference 2>&1 | grep -E "Connected|Tools discovered"'
```
