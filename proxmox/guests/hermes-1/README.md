...
The Hermes runtime `.env` is intentionally not in Git. It contains the dashboard break-glass authentication values and API-server token. A root-readable recovery copy of those client credentials is stored at `/root/hermes-access.txt` inside the backed-up VM. The separate Cloudflare token used by Caddy is stored in `/srv/caddy/.env` at runtime and is therefore also captured by VM backups; do not add this infrastructure credential to the client-access file. Its authoritative copy belongs in 1Password under `hermes - kelch.io`.
