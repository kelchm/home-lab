...
This directory intentionally contains no Flux resources. Merging this change lets the parent `cluster-apps` Kustomization prune the Connector, operator, namespace, repository, and OAuth secret. After Flux confirms deletion, remove the stale `lan-subnet-router` and `tailscale-operator` machines, delete the unused OAuth client, and remove `tag:k8s` plus `tag:k8s-operator` from the Tailnet policy.
