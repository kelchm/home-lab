...
The SOPS-encrypted settings Secret is the only source of truth. The dashboard, management API, OAuth server, Better Auth, and discovery are unavailable externally, and read-only mode rejects accidental mutations from inside the cluster.
...
Bearer-key and group metadata remain available for activity attribution, but full tool arguments and results are not stored by MCPHub.
